...
- (id)init
{
    ...
    msids =[[NSMutableArray alloc]init];
...
- (void)onSessionInitiate:(XMPPIQ *)iq sid:(NSString *)sid
{
    ...
    [self.delegate didReceiveSessionMsg:sid type:@"session-initiate" data:jsonDict :nil];
    
}
// Called when a source add message is received
- (void)onSourceAdd:(XMPPIQ *)iq sid:(NSString *)sid
{
    // Parse SDP
    ...
    // post the message to delegate
    [self.delegate didReceiveSessionMsg:nil type :@"source-add" data:jsonDict:msids];
...
- (void)onSessionAccept:(XMPPIQ *)iq sid:(NSString *)sid
{
    ...
    // post the message to delegate
    [self.delegate didReceiveSessionMsg:sid type:@"session-accept" data:jsonDict:nil];
}
...
- (BOOL)xmppStream:(XMPPStream *)sender didReceiveIQ:(XMPPIQ *)iq
{
    ...
    {
        [self onSourceAdd:iq sid:[jingleElement attributeStringValueForName:@"sid"]];
    }
