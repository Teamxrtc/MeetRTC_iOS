 ...
 withConnectionTimeout:(NSTimeInterval)connectionTimeout
{
    if (!_isConnected && !_isConnecting) {
        ...
        NSArray *cookieArray = [[NSHTTPCookieStorage sharedHTTPCookieStorage] cookiesForURL:[NSURL URLWithString:handshakeUrl]];
        for (NSHTTPCookie *cookie in cookieArray) {
            [[NSHTTPCookieStorage sharedHTTPCookieStorage] deleteCookie:cookie];
        }
        
        // make a request
        DEBUGLOG(@"Before making a request");
...
{
    DEBUGLOG(@"Starting Send message");
...
- (void) sendEvent:(NSString *)eventName withData:(id)data andAcknowledge:(SocketIOCallback)function
{
    ...
    // To be fixed: Manish
    //NSString *packetstring = [SocketIOJSONSerialization JSONStringFromObject:dict error:nil];
    //packet.data = [packetstring stringByReplacingOccurrencesOfString:@"\\\\\\\\/" withString:@"\\/"];;
...
- (void) sendHeartbeat
{
   ...
   // NSLog(@"sending registration data : %@", values);
    ...
    //[self sendEvent:@"reg_client_message" withData:values andAcknowledge:cb];
    //NSLog(@"After Sending");
    //uptil here

    SocketIOPacket *packet = [[SocketIOPacket alloc] initWithType:@"heartbeat"]; //commented by self both lines
    [self send:packet];
}
...
- (void) connection:(NSURLConnection *)connection didFailWithError:(NSError *)error 
{
    ...
    if ([_delegate respondsToSelector:@selector(socketIO:onError:)]) {
        NSMutableDictionary *errorInfo = (NSMutableDictionary*)[NSDictionary dictionaryWithObject:error forKey:NSLocalizedDescriptionKey];
